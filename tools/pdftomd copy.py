...
def pdf_to_markdown(
    ...
    try:
        ...
        out_file = Path(output_dir) / f"{base_name}.{output_format}"
        print(f"[AGENT-CALL] ✅ pdf_to_markdown 完成: {pdf_path} → {out_file}", file=sys.stderr)
        return f"{out_file}"
...
def extract_text_and_images(
    ...
    try:
        ...
        all_links = list(dict.fromkeys(md_imgs + html_imgs))     # 去重并保持顺序
