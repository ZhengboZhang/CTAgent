...
"""
MCP Server: Word Writer
Exposes tools to write academic-style content into a Word (.docx) document:
- Initialize document with page layout and default styles
- Write document title (centered, large font)
- Write headings (H1/H2/H3) with proper fonts and spacing
- Write body paragraphs with 1.5 line spacing and first-line indent
- Write tables (with headers, rows, optional column widths)
- Insert images (optional width, caption)
- Provide a recommended prompt to guide LLMs to use the tools in sequence

Dependencies:
  pip install mcp python-docx Pillow lxml
"""
...
def get_word_writing_prompt() -> str:
    ...
    return (
        ...
        "5) 在输出中清楚说明你已完成了哪些写入（例如：已写入Title、H1、3段正文、1个表格、1张图片），并提供最终文档路径。\n"
