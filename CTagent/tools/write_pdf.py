...
"""
MCP Server: PDF Writer (Linux only)

Provides tools to:
- Convert Word (.docx/.doc) to PDF on Linux via LibreOffice (soffice --headless)
- Return a workflow prompt instructing to write Word first (using word-writer tools), then convert to PDF
- Optionally delete the original Word file
